...
required check as passed. When stack standards are in scope, it MUST map a
validated selection record to repository-declared available evidence, preserve
selected rule IDs and overrides, and report a gap rather than invent a command
or coverage claim when selection or required evidence is absent.
