...
- Primary GitHub issue: pending. The exact issue intake dry-run found no duplicate, but the active runtime denied publishing the reviewed payload to `jizzoe/joericearchitect-ai-skills`; planning review must not pass until the issue exists and tracking metadata is valid.
...
- Dependencies: existing controller schema, canonical correction ledger, `base-code-review`, `base-verification-loop`, OpenSpec lifecycle actions, and generated adapter drift checks.
