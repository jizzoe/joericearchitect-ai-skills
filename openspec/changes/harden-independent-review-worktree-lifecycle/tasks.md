...
- [x] 4.7 Add a fixed parent-runtime strict Codex transport that binds the exact
  ...
  content hash and OS-backed signer or ownership/mode trust, inner read-only
  profile, result artifact, expiration, and owned cleanup. Reject
  caller-selected executable paths and keep authorized-degraded recovery
  separate. Depends on: 4.6.
...
- [x] 5.1 Run focused lifecycle and adapter tests, the full Node suite,
  ...
  Depends on: 3.3, 4.3.
- [x] 5.2 Obtain a fresh exact-head independent review under the repaired
