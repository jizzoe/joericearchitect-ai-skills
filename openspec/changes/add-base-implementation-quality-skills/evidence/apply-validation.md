...
- `node --test evals/skills/implementation-quality/run-fixtures.test.mjs`
  passed 19 tests with 0 failures after the strict-review corrections for
  production-gate applicability, current evidence bindings, correction
  exhaustion, check/evidence result agreement, and local-finding resolution.
...
- `rg --files scripts evals -g '*.test.mjs' | sort | xargs node --test`
  passed 187 tests with 0 failures, skips, cancellations, or todos in the
