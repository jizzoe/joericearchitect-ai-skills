...
- `node --test evals/skills/implementation-quality/run-fixtures.test.mjs`
  passed 21 tests with 0 failures after the strict-review corrections for
  ...
  not-applicable reasons that cannot bypass applicable checks.
...
- `rg --files scripts evals -g '*.test.mjs' | sort | xargs node --test`
  passed 189 tests with 0 failures, skips, cancellations, or todos in the
