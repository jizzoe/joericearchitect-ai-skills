...
- `node --test evals/skills/implementation-quality/run-fixtures.test.mjs`
  passed 24 tests with 0 failures after the strict-review corrections for
  ...
  reviewed-path coverage, and consistent result status/readiness state.
...
- `rg --files scripts evals -g '*.test.mjs' | sort | xargs node --test`
  passed 192 tests with 0 failures, skips, cancellations, or todos in the
