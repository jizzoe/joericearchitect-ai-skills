...
The next corrected-head reviewer identified `IR-001` parent-authority
provenance: the initial Codex transport would have executed repository-
controlled host JavaScript outside the managed sandbox. The bounded correction
materializes and validates an exact-head regular-file archive inside the
sandbox and elevates only host-owned environment/reviewer executables with
