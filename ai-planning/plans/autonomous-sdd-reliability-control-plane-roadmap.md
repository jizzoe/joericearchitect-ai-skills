...
coordination milestone is added. M2-S1 and M2-S2 are delivered and archived;
M2-S3 is next. Every slice still requires its own accepted brief and explicit
authorization.
...
M2-S1 (`prove-autonomous-sdd-vertical-slice`) and M2-S2
(`add-autonomous-sdd-local-execution-backend`) are delivered and archived. Begin
M2-S3 (`add-autonomous-sdd-run-status-and-recovery`) under a new exact
