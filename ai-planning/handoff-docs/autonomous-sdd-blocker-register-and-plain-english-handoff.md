...
| 2026-08-22 | `repair-v2-controller-initialization`, Sync PR #183 | `openspec-validate` rejected active `repair-m1-s2-v2-terminalization` | A **modified OpenSpec requirement** is a complete replacement copy, not a small patch. Sync added the new initializer scenario to the living shared-runtime requirement. The still-active M1-S2 repair's older copy does not include that scenario, so validation correctly refuses to make it appear that M1-S2 deletes the new behavior. | Permanent Sync-coordination repair; intentional specification-integrity stop | Yes. Finished milestones should still reject an active change that would silently remove a newer living scenario. The missing detection or coordinated update is a permanent workflow repair. | Before this Sync can merge, add only the identical initializer scenario to M1-S2's matching modified delta, validate both active changes together, and deliver that compatibility edit through authority that explicitly permits touching M1-S2. Longer term, make Sync preflight enumerate active overlapping modified requirements and either update them atomically under shared authority or stop before creating the Sync PR. | PR #183: linkage and Project audit passed; OpenSpec CI failed only because `repair-m1-s2-v2-terminalization/specs/shared-sdd-runtime-distribution/spec.md` omits `V2 delivery is initialized through the installed runtime`. Current bridge forbids unrelated-change edits, so no M1-S2 file was changed. |
| 2026-08-22 | M1-S2 compatibility PR #184, post-merge branch reconciliation | `remote-branch-missing-after-merge` | PR #184 squash-merged the exact reviewed head, but GitHub removed its remote topic branch even though the merge request did not ask to delete it. The restricted repository read did not expose a usable `delete_branch_on_merge` setting, so the deletion policy could not be proven before merge. | External lifecycle-policy gap; unexpected but recoverable side effect | Yes. A repository configured to remove merged branches can still do this after all milestones unless the delivery adapter preflights the setting or records and restores an explicitly retained branch. | Re-push only the still-present clean local branch at the exact merged topic head, verify the remote ref, and keep remote deletion forbidden. Amend M4-S1 planning with repository auto-delete policy preflight and an exact non-secret branch-retention receipt. | Resolved for this exact branch: PR #184 merged as `cdd9edad6331589dc1c4d97e3b1ce1b252004933`; the remote ref was restored at exact topic head `aa000ae8f75806337429cbf4ec941a9de614c90a` without force or deletion. The M4-S1 planning repair remains required. |
| 2026-08-22 | `repair-v2-controller-initialization` Sync PR #183, post-merge branch reconciliation | `remote-branch-missing-after-merge` | The same unproven GitHub branch-deletion policy removed the Sync topic branch after its exact reviewed head was squash-merged, even though the authorized request again specified no remote deletion. | External lifecycle-policy gap; repeated symptom of the same root cause | Yes. This repeat confirms that branch retention needs a preflight and receipt-backed restoration path rather than relying on the merge request alone. | Preserve the merged PR and main commit, re-push only the exact clean local Sync head without force, verify it, and carry the common M4-S1 planning repair forward. | Resolved for this exact branch: PR #183 merged as `008e734e0b5dd635063dbd113bdaa12e0f284ed2`; the remote Sync ref was restored at exact topic head `2f7b762c679e53700e98a062a2150c15247284a7` without force or deletion. |
| 2026-08-22 | `repair-v2-controller-initialization`, post-Sync Archive exactness gate | `delta-living-description-mismatch` | Every delta scenario reached the living specs, but two complete `MODIFIED` requirement descriptions did not. `Complete delivery intake creates durable controller context` dropped the word `initialization`, and `Every declared helper is dispatchable through one contract` became the malformed phrase `terminalization operation MUST or v2 delivery initialization operation MUST`. Green schema validation could not detect this semantic text loss. | Permanent Sync-verification repair; intentional Archive safety stop | Yes. Schema-valid Markdown can still omit or corrupt accepted requirement prose, so Archive must continue to require exact delta-description and scenario reflection. | Use correction attempt 1 to replace only the two affected living requirement descriptions with their accepted delta descriptions, retain all unrelated living requirements and scenarios, rerun the exact semantic comparison and strict validation, and deliver the correction through a linked PR before Archive. Add description-aware comparison coverage to the M4-S2 planning correction. | Resolved by correction PR #185, squash-merged as `91b32c951b9eb6aecab9e62fbb8beb3d5559e31e`; exact comparison and strict OpenSpec validation passed before delivery. |
| 2026-08-22 | Sync-exactness correction PR #185, first merge attempt | `401 Requires authentication` after a successful bound repository probe | The exact merge write lost access to the host GitHub credential even though the immediately preceding non-secret repository probe succeeded. The PR remained open and `main` unchanged. | Restricted/host credential-boundary interruption; recoverable exact retry | The restricted runtime should remain credential-isolated, but a finished host-operation envelope must bind the exact write and produce a durable non-secret receipt instead of exposing intermittent credential visibility. | Reread the exact PR and `main`, repeat the safe probe, then retry only PR #185's exact-head squash merge through the approved host permission boundary. Carry the authenticated-host operation-envelope repair into M4-S1 planning. | Resolved: host-bound retry merged exact head `07ad06fccbd839dd211ec11653bb5afb3cd31aa2` as `91b32c951b9eb6aecab9e62fbb8beb3d5559e31e`; no unbound GitHub action was performed. |
| 2026-08-22 | Sync-exactness correction PR #185, post-merge branch reconciliation | `remote-branch-missing-after-merge` | GitHub again removed a retained topic branch after squash merge, reproducing the same repository lifecycle-policy gap already observed for PRs #184 and #183. | External lifecycle-policy gap; third symptom of the same root cause | Yes, until the merge adapter preflights and receipts repository auto-delete behavior. | Restore only the exact clean reviewed head without force and retain the shared M4-S1 planning repair. | Resolved for this exact branch: remote `fix/181-sync-delta-description-exactness` was restored at `07ad06fccbd839dd211ec11653bb5afb3cd31aa2` without force or deletion. |
| 2026-08-22 | `repair-v2-controller-initialization` Archive PR #186, post-merge branch reconciliation | `remote-branch-missing-after-merge` | GitHub removed the explicitly retained Archive topic branch after the fourth consecutive squash merge in this recovery sequence. | External lifecycle-policy gap; fourth symptom of the same root cause | Yes, until the repository policy is preflighted and exact branch-retention evidence is part of the delivery contract. | Preserve the delivered Archive commit, restore only the exact reviewed Archive topic head without force, and retain the common M4-S1 planning repair. | Resolved for this exact branch: PR #186 merged as `2b0caed0b59305102bfa33e76f554143d9c4a91e`; remote `chore/181-archive-v2-controller-initialization` was restored at `aec22de17bd22eb2735d9d95641ce03feecb0058`. |
| 2026-08-22 | `repair-v2-controller-initialization` final remote-retention reconciliation | `remote-branch-missing-after-merge` | The implementation topic ref from earlier PR #182 was also absent while the four newer repair refs had been restored, showing the same auto-delete policy affected the complete lifecycle rather than only this session's merges. | External lifecycle-policy gap; earlier symptom reconciled at final cleanup | Yes, for any delivery that requires audit-time remote branch retention. | Recreate only the exact reviewed implementation ref from PR #182's durable topic head, without force, and verify all five remote refs together. | Resolved: remote `repair/v2-controller-initialization` was restored at `4dd7a0ddafde045002eaaac882d4057be9e33b4f`; all five repair remote refs now match their reviewed topic heads. |
...
| 2026-08-22 | `repair-m1-s2-v2-terminalization`, installed initializer admission | `legacy-inventory-ambiguous` | `rootCauseId=initializer-self-inventory-v1`; the installed wrapper passes the entire repository-common `.git/sdd-delivery-runs` tree to legacy inventory. The controller-first protocol writes its schema-5 pending checkpoint before admission, but the legacy decoder accepts only schema 1–4 controller records and recursively treats every JSON file as legacy input. It therefore classifies the initializer's own pending checkpoint as ambiguous and blocks itself. The first attempt also inventoried the copied request file; moving only that exact copy outside the scan root and retrying produced the same stop, proving the schema-5 checkpoint is independently sufficient. | Permanent implementation defect plus intentional fail-closed safety stop; `expectedStop=false`; `escapedGate=focused tests invoked the initializer without the installed wrapper's real legacy-directory binding` | The fail-closed response to a genuinely unknown legacy record should remain. This exact self-block would affect every installed-runtime initialization after all currently written milestones until repaired. | `temporaryUntil=a separately authorized pre-v2 repair is delivered and installed`; `permanentRepair=inventory only actual legacy controller records, exclude the current schema-5 controller and non-controller JSON without weakening unknown-legacy rejection, and add an installed-wrapper integration test against a real Git-common controller root`. Then resume the same deterministic pending controller and require matching admission identities. | Pending new repair authority. Exact terminal reconciliation receipts cover both active-looking v1 bootstrap controllers; the v2 repository has no active admission, and controller `controller-b571bcbd728031452e90fff28350e1ec` remains safely pending with claim `claim-b571bcbd728031452e90fff28350e1ec` not created. |
| 2026-08-22 | `stabilize-autonomous-sdd-bootstrap-and-cutover-plan`, installed initializer admission after the self-inventory repair | `legacy-inventory-ambiguous` | `rootCauseId=initializer-terminal-controller-inventory-v1`; the repaired inventory correctly excluded its own pending schema-5 checkpoint, but still treated a prior schema-5 controller as ambiguous even though immutable terminalization, claim-release, repository, selected-change, and v2 archive evidence proved that controller finished. | Permanent implementation defect plus intentional fail-closed safety stop; `expectedStop=false`; `escapedGate=installed-wrapper coverage lacked a real Git-common prior terminal controller plus matching v2 archive state` | A genuinely pending, active, mismatched, malformed, or ambiguous controller must still block. A fully evidence-proven terminal controller should not block any later admission. | `temporaryUntil=repair-v2-initializer-terminal-controller-inventory is delivered and installed`; `permanentRepair=accept a prior schema-5 controller only when immutable controller identity, repository, selected change, terminalization receipt, claim release, and archived run agree; retain fail-closed behavior for every mismatch and cover the installed wrapper against real Git-common and v2 archive state`. | Resolved: issue #193, implementation PR #194 (`e502ab874b5bd59ab83758f459c1e9b001df554e`), Sync PR #195 (`1f923c49`), Archive PR #196 (`138b2212f33af4dc97abaedff93d3d7e4558c61e`), and runtime `runtime-e0e9a50a042b` / digest `e0e9a50a042bae3ba43f842ced29799546860edf09464dc49557a3aed70c274a`. The pending planning controller then resumed and acquired its matching v2 claim. |
| 2026-08-22 | `repair-v2-initializer-terminal-controller-inventory`, implementation/Sync/Archive post-merge reconciliation | `remote-branch-missing-after-merge` | `rootCauseId=github-retained-branch-auto-delete-v1`; repository policy again removed all three topic refs after squash merge even though authorization required retaining them. This is another symptom of the branch-policy gap already recorded for PRs #182-#186, not a new architecture failure. | External lifecycle-policy gap; `expectedStop=false`; `escapedGate=M4-S1 does not yet preflight automatic branch deletion or require a retention receipt` | Yes, until M4-S1 implements the exact policy preflight and receipt-backed retention path. | `temporaryUntil=exact reviewed local heads are re-pushed without force`; `permanentRepair=M4-S1 exact merge-policy/auto-delete preflight plus post-merge ref verification and exact non-secret retention receipt`. | Resolved for this delivery: remote `repair/v2-initializer-terminal-controller-inventory`, `chore/sync-v2-initializer-terminal-controller-inventory`, and `chore/archive-v2-initializer-terminal-controller-inventory` were restored at their exact reviewed topic heads. No remote branch was deleted by the controller. |

### Causal metadata contract

New or updated rows should carry these fields in the plain-English cause,
classification, and repair cells when applicable:

- `rootCauseId`: stable identifier shared by repeated symptoms;
- `expectedStop`: whether a correctly completed system should still stop here;
- `temporaryUntil`: exact evidence or delivery that unblocks the current run;
- `permanentRepair`: owning slice/change that prevents recurrence; and
- `escapedGate`: the missing test or preflight that allowed the defect through.

Chronological rows remain immutable evidence. Shared `rootCauseId` values make
causal counts distinct from interruption counts.
