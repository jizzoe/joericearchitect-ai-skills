...
export function validateFindingDispositions({ findings, dispositions, correctionAttempts = 0 }) {
  ...
  for (const finding of findings) {
    ...
    if (disposition.kind === "objective-fix" && correctionAttempts >= 3) return pause("correction-limit-exhausted", finding.id);
  }
  return { allowed: true, classification: "ready", issues: [] };
}

export function nextReviewState({ priorHead, currentHead, findings, dispositions, correctionAttempts = 0 }) {
  if (priorHead !== currentHead) return { state: "rereview-required", reason: "head-changed" };
  const result = validateFindingDispositions({ findings, dispositions, correctionAttempts });
