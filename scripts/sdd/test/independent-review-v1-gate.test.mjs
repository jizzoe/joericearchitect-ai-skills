...
test("authorized degraded result requires strict unavailable evidence and exact risk acceptance", () => {
  ...
  const accepted = validateIndependentReviewV1({ reviewer: strictReviewer, degradedReviewer, authorization, selectedEntry: "change", implementerSession: "implementer", reviewPackage, reviewResult: degraded, applyEvidence, now: "2026-08-13T04:00:00.000Z" });
  assert.equal(accepted.allowed, true, JSON.stringify(accepted));
  assert.equal(validateIndependentReviewV1({ reviewer: strictReviewer, degradedReviewer, authorization: {}, selectedEntry: "change", implementerSession: "implementer", reviewPackage, reviewResult: degraded, applyEvidence, now: "2026-08-13T04:00:00.000Z" }).issues[0].code, "degraded-independent-review-not-authorized");
  assert.equal(validateIndependentReviewV1({ reviewer: strictReviewer, degradedReviewer, authorization, selectedEntry: "change", implementerSession: "implementer", reviewPackage, reviewResult: { ...degraded, strictUnavailable: { ...degraded.strictUnavailable, headCommit: baseCommit } }, applyEvidence, now: "2026-08-13T04:00:00.000Z" }).issues[0].code, "independent-review-result-degraded-evidence-invalid");
});
