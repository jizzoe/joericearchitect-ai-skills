...
test("delivery gate accepts one already-durable objective correction", () => {
  ...
  const authorization = { targets: ["workspace:reports"], allowedMutations: ["run-lifecycle-action"], derivedTargets: { queue: ["change"], selectedEntry: "change", repository: "owner/repository" }, degradedIndependentReview: { enabled: true, change: "change", transitions: ["merge-pr"], expiresAt: "2026-08-14T00:00:00.000Z", riskReason: "exact synthetic fallback", fallbackBoundary: "fresh-separated-reviewer-only", baseCommit, headCommit: initialHead, manifestDigest: initialManifest, allowDerivedObjectiveCorrections: true, derivedCorrections: [correction] } };
