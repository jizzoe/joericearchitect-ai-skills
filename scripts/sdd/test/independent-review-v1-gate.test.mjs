...
test("normalized v1 review result requires current Apply evidence and preserves dispositions", () => {
  ...
  const objectiveResult = validateIndependentReviewV1({ reviewer: configured, implementerSession: "implementer", reviewPackage, reviewResult: { ...result, status: "failed", findings: [objective] }, applyEvidence, dispositions: [{ findingId: "objective-1", kind: "objective-fix", evidence: "review evidence", failureSignature: "objective-signature" }] });
  ...
  assert.equal(validateIndependentReviewV1({ reviewer: configured, implementerSession: "implementer", reviewPackage, reviewResult: { ...result, status: "failed", findings: [objective] }, applyEvidence, dispositions: [{ findingId: "objective-1", kind: "warning", evidence: "unsupported relabel" }] }).issues[0].code, "independent-review-disposition-incompatible");
});
...
test("operation checker accepts only a durable exact v1 record", () => {
  ...
  const authorization = { targets: ["workspace:reports"], allowedMutations: ["run-lifecycle-action"], qualityProfile: "production-rapid", derivedTargets: { queue: ["change"], selectedEntry: "change", repository: "owner/repository" } };
...
test("authorized degraded result requires strict unavailable evidence and exact risk acceptance", () => {
  ...
  const missingAuthenticityLimitation = { ...degraded, capabilityLedger: { ...degraded.capabilityLedger, unavailable: degraded.capabilityLedger.unavailable.filter((item) => item !== "authenticatedParentLaunchEvidence") } };
  assert.equal(validateIndependentReviewV1({ reviewer: strictReviewer, degradedReviewer, authorization, selectedEntry: "change", implementerSession: "implementer", reviewPackage, reviewResult: missingAuthenticityLimitation, strictUnavailableResult: strictUnavailable, applyEvidence, now: "2026-08-13T04:00:00.000Z" }).issues[0].code, "independent-review-result-degraded-evidence-invalid");
...
test("delivery gate accepts one already-durable objective correction", () => {
  const { reviewPackage, result } = make();
  const initialHead = baseCommit;
  const initialManifest = "f".repeat(64);
  const failureSource = { kind: "independent-review", reviewRecordId: "review-fixture", findingId: "review-fixture", severity: "high", evidence: "scripts/sdd/independent-review.mjs", transition: "merge-pr" };
  const correction = { id: "correction-1", change: "change", attempt: 1, failureSource, failureSignature: "independent-review/review-fixture/scripts/sdd/independent-review.mjs/merge-pr", classification: "objective-fix", behaviorPreserving: true, current: true, ancestryVerified: true, evidenceReference: "checkpoint:correction-1", baseCommit, previousHead: initialHead, previousManifestDigest: initialManifest, headCommit, manifestDigest: reviewPackage.manifestDigest };
  const strictUnavailable = { ...result, reviewRecordId: "strict-unavailable-derived", executionId: "strict-execution-derived", status: "unavailable", unavailableCode: "strict-runtime-unavailable", attestation: { ...result.attestation, nonInteractive: false, isolatedContext: false, freshContext: false, readOnly: false } };
  const degraded = { ...result, reviewRecordId: "degraded-derived", executionId: "degraded-execution-derived", reviewer: { type: "fixture-degraded", identity: "fresh-degraded", adapter: "fixture-degraded" }, attestation: { ref: "degraded-attestation", nonInteractive: true, isolatedContext: false, freshContext: true, readOnly: false }, assuranceLevel: "authorized-degraded", capabilityLedger: { enforced: ["githubMutation", "deployment", "release", "externalSend", "delegatedMutation"], unavailable: ["workspaceWrite", "gitWrite", "credentialAccess", "authenticatedNetwork", "authenticatedParentLaunchEvidence", "hostPinnedReviewerExecutableIdentity"], instructionConstrained: [] }, strictUnavailable: { reviewRecordId: strictUnavailable.reviewRecordId, executionId: strictUnavailable.executionId, adapter: "fixture", status: "unavailable", unavailableCode: strictUnavailable.unavailableCode, baseCommit, headCommit, manifestDigest: reviewPackage.manifestDigest }, degradedAuthorization: { change: "change", transition: "merge-pr", expiresAt: "2026-08-14T00:00:00.000Z", riskReason: "exact synthetic fallback", fallbackBoundary: "fresh-separated-reviewer-only" } };
  const applyEvidence = { reference: "apply-derived", current: true, headCommit, completedAt: "2026-08-13T03:00:00.000Z", validationEvidence: ["tests"] };
  const issue = { entry: "change", kind: "issue", id: "8", repository: "owner/repository", evidence: { reference: "issue", current: true } };
  const branch = { entry: "change", kind: "branch", id: "feature/change", repository: "owner/repository", baseBranch: "main", headCommit, evidence: { reference: "branch", current: true, headCommit } };
  const pr = { entry: "change", kind: "pr", id: "7", repository: "owner/repository", baseBranch: "main", headCommit, evidence: { reference: "pr", current: true, headCommit } };
  const reviewRecord = { id: degraded.reviewRecordId, entry: "change", transition: "merge-pr", evidence: {}, reviewPackage, result: degraded, dispositions: [], strictUnavailable: degraded.strictUnavailable, degradedAuthorization: degraded.degradedAuthorization, capabilityLedger: degraded.capabilityLedger };
  const strictRecord = { id: strictUnavailable.reviewRecordId, entry: "change", transition: "merge-pr", current: true, evidenceReference: "strict:derived", reviewPackage, result: strictUnavailable };
  const steps = ["issue", "branch", "pr", "merge-pr", "sync-change", "archive-change", "delete-merged-topic-branch"].map((id, index) => ({ id, status: index < 3 ? "complete" : "pending", evidence: index < 3 ? { present: true, current: true } : undefined }));
  const checkpoint = { selectedEntry: { name: "change", records: [issue, branch, pr], applyEvidenceRecords: [applyEvidence], reviewRecords: [reviewRecord], strictUnavailableRecords: [strictRecord], correctionAnchor: { baseCommit, headCommit: initialHead, manifestDigest: initialManifest }, correctionRecords: [correction] }, steps };
  const authorization = { expiresAt: "2026-08-14T00:00:00.000Z", targets: ["workspace:reports"], allowedMutations: ["run-lifecycle-action"], qualityProfile: "production-rapid", derivedTargets: { queue: ["change"], selectedEntry: "change", repository: "owner/repository" }, degradedIndependentReview: { enabled: true, change: "change", transitions: ["merge-pr"], expiresAt: "2026-08-14T00:00:00.000Z", riskReason: "exact synthetic fallback", fallbackBoundary: "fresh-separated-reviewer-only", baseCommit, headCommit: initialHead, manifestDigest: initialManifest, allowDerivedObjectiveCorrections: true, derivedCorrections: [correction] } };
  const config = { independentReviewer: reviewer, degradedIndependentReviewer: { type: "fixture-degraded", identity: "fresh-degraded", enabled: true, attestation: { ref: "degraded-attestation", nonInteractive: true, freshContext: true } } };
  const request = { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:7", selectedEntry: "change", derivedRecord: pr, headCommit, baseCommit, evidenceCurrent: true, evidenceReference: "pr", evidenceHeadCommit: headCommit, recovery: "re-read state", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer: { type: "fixture-degraded", identity: "fresh-degraded" }, strictReviewer: { type: "fixture", identity: "fresh-reviewer" }, checkpoint, applyEvidence, independentReviewPackage: reviewPackage, independentReviewResult: degraded, reviewDispositions: [], correctionAttempts: 1, derivedCorrection: true };
  const accepted = checkOperationAuthorization({ authorization, runtime: { permittedOperations: ["run-lifecycle-action"] }, config, request, now: "2026-08-13T04:00:00.000Z" });
  assert.equal(accepted.allowed, true, JSON.stringify(accepted));
});
