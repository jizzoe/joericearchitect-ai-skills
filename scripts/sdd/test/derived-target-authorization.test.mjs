...
test("production-rapid delivery requires current independent review evidence", () => {
  ...
  const checkpoint = { ...lifecycleCheckpoint([issueRecord, branchRecord, record], "merge-pr"), selectedEntry: { ...lifecycleCheckpoint([issueRecord, branchRecord, record], "merge-pr").selectedEntry, reviewRecords: [{ id: "review-1", entry: "first-change", transition: "merge-pr", evidence }] } };
  ...
  assert.equal(checkOperationAuthorization({ authorization, runtime, config, request: { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, checkpoint: derivedCheckpoint, headCommit: headSha, baseCommit: baseSha, applyEvidence, evidenceCurrent: true, evidenceReference: "pr-evidence", evidenceHeadCommit: headSha, recovery: "recover", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer, independentReviewInput: reviewInput, reviewRepositoryPath: repositoryPath, independentReviewEvidence: evidence } }).issues[0]?.code, "independent-review-evidence-not-durable");
