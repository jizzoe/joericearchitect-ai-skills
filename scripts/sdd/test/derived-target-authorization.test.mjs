...
const requiredOpenSpecArtifacts = ["proposal.md", "design.md", "tasks.md", "specs/bounded-autonomous-execution/spec.md"];
const config = { reviewRepositoryPath: repositoryPath, independentReviewer: configuredReviewer, requiredOpenSpecArtifacts };
...
const issueRecord = { entry: "first-change", kind: "issue", id: "72", repository: "owner/repository", evidence: { reference: "issue-evidence", current: true } };
const branchRecord = { entry: "first-change", kind: "branch", id: "feature/first-change", repository: "owner/repository", baseBranch: "main", headCommit: headSha, evidence: { reference: "branch-evidence", current: true, headCommit: headSha } };
const lifecycleSteps = ["issue", "branch", "pr", "merge-pr", "sync-change", "archive-change", "delete-merged-topic-branch"];
function lifecycleCheckpoint(records, pending) {
  const pendingIndex = lifecycleSteps.indexOf(pending);
  return { selectedEntry: { name: "first-change", records }, steps: lifecycleSteps.map((id, index) => ({ id, status: index < pendingIndex ? "complete" : "pending", evidence: index < pendingIndex ? { present: true, current: true } : undefined })) };
}
...
test("derived declarations require durable records for Sync as well as high-impact actions", () => {
  ...
  assert.equal(checkOperationAuthorization({ authorization: { ...authorization, targets: ["sync:first-change"] }, runtime, request: { ...request, derivedRecord: undefined, selectedEntry: undefined, checkpoint: lifecycleCheckpoint([issueRecord, branchRecord, record, sync], "sync-change") } }).issues[0]?.code, "derived-record-not-durable");
  assert.equal(checkOperationAuthorization({ authorization, runtime, request: { ...request, checkpoint: lifecycleCheckpoint([issueRecord, branchRecord, record, sync], "sync-change") } }).allowed, true);
  assert.equal(checkOperationAuthorization({ authorization, runtime, request: { ...request, derivedRecord: record, target: "pr:42", checkpoint: lifecycleCheckpoint([issueRecord, branchRecord, record], "sync-change") } }).issues[0]?.code, "derived-checkpoint-schema-invalid");
});
...
test("preserves exact target authorization without derived declarations", () => {
  ...
  assert.equal(result.allowed, true, JSON.stringify(result));
  assert.equal(checkOperationAuthorization({ authorization, runtime, config, request: { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, checkpoint: lifecycleCheckpoint([], "merge-pr"), headCommit: headSha, evidenceCurrent: true, recovery: "recover" } }).issues[0]?.code, "derived-checkpoint-schema-invalid");
  assert.equal(checkOperationAuthorization({ authorization: { ...authorization, targets: ["pr:42"] }, runtime, config, request: { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", checkpoint: { selectedEntry: { name: "first-change", records: [record] }, steps: [{ id: "merge-pr", status: "pending" }] }, headCommit: headSha, evidenceCurrent: true, recovery: "recover" } }).issues[0]?.code, "derived-checkpoint-schema-invalid");
});
...
test("production-rapid delivery requires current independent review evidence", () => {
  ...
  const applyEvidence = { reference: "apply-1", current: true, headCommit: headSha, completedAt: "2026-08-12T11:00:00.000Z", validationEvidence: ["tests"] };
  const reviewInput = { baseCommit: baseSha, headCommit: headSha, diff: accumulatedDiff, openspecArtifacts: requiredOpenSpecArtifacts, validationEvidence: applyEvidence.validationEvidence };
  const evidence = { reviewer: { type: "fixture-reviewer", identity: "fresh-reviewer" }, executionRef: "run", invocationRef: "fixture", reviewedBase: baseSha, reviewedHead: headSha, inputManifest: immutableReviewManifest(reviewInput), applyEvidenceRef: "apply-1", timestamp: "2026-08-12T12:00:00.000Z", findings: [], dispositions: [], finalStatus: "clear" };
  const checkpoint = { ...lifecycleCheckpoint([issueRecord, branchRecord, record], "merge-pr"), selectedEntry: { ...lifecycleCheckpoint([issueRecord, branchRecord, record], "merge-pr").selectedEntry, applyEvidenceRecords: [applyEvidence], reviewRecords: [{ id: "review-1", entry: "first-change", transition: "merge-pr", evidence }] } };
  assert.equal(prepareIndependentReview({ reviewer, implementerSession: "implementer", reviewInput }).allowed, true);
  const result = checkOperationAuthorization({ authorization, runtime, config, request: {
    profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, headCommit: headSha, baseCommit: baseSha, independentReviewInput: reviewInput, reviewRepositoryPath: repositoryPath, applyEvidence, evidenceCurrent: true, evidenceReference: "pr-evidence", evidenceHeadCommit: headSha, recovery: "re-read PR and checkpoint", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer, checkpoint, reviewRecordId: "review-1", independentReviewEvidence: evidence
  } });
  assert.equal(result.allowed, true, JSON.stringify(result));
  assert.equal(checkOperationAuthorization({ authorization, runtime, config, request: { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, checkpoint, headCommit: headSha, baseCommit: baseSha, independentReviewInput: reviewInput, reviewRepositoryPath: repositoryPath, applyEvidence: { ...applyEvidence, completedAt: "2026-08-12T10:00:00.000Z" }, evidenceCurrent: true, evidenceReference: "pr-evidence", evidenceHeadCommit: headSha, recovery: "recover", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer, reviewRecordId: "review-1", independentReviewEvidence: evidence } }).issues[0]?.code, "independent-review-apply-evidence-not-durable");
  assert.equal(checkOperationAuthorization({ authorization, runtime, config, request: { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, checkpoint: { ...checkpoint, selectedEntry: { ...checkpoint.selectedEntry, applyEvidenceRecords: [applyEvidence, { ...applyEvidence, reference: "apply-2" }] } }, headCommit: headSha, baseCommit: baseSha, independentReviewInput: reviewInput, reviewRepositoryPath: repositoryPath, applyEvidence, evidenceCurrent: true, evidenceReference: "pr-evidence", evidenceHeadCommit: headSha, recovery: "recover", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer, reviewRecordId: "review-1", independentReviewEvidence: evidence } }).issues[0]?.code, "derived-checkpoint-not-valid");
  assert.equal(checkOperationAuthorization({ authorization, runtime, config, request: { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, checkpoint, headCommit: headSha, baseCommit: baseSha, independentReviewInput: { ...reviewInput, openspecArtifacts: ["proposal.md"] }, reviewRepositoryPath: repositoryPath, applyEvidence, evidenceCurrent: true, evidenceReference: "pr-evidence", evidenceHeadCommit: headSha, recovery: "recover", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer, reviewRecordId: "review-1", independentReviewEvidence: evidence } }).issues[0]?.code, "independent-review-input-evidence-mismatch");
  assert.equal(checkOperationAuthorization({ authorization, runtime, config, request: { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, checkpoint, headCommit: headSha, baseCommit: baseSha, independentReviewInput: { ...reviewInput, validationEvidence: ["stale-tests"] }, reviewRepositoryPath: repositoryPath, applyEvidence, evidenceCurrent: true, evidenceReference: "pr-evidence", evidenceHeadCommit: headSha, recovery: "recover", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer, reviewRecordId: "review-1", independentReviewEvidence: evidence } }).issues[0]?.code, "independent-review-input-evidence-mismatch");
  assert.equal(checkOperationAuthorization({ authorization, runtime, config, request: { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, checkpoint: { ...lifecycleCheckpoint([issueRecord, branchRecord, record], "pr"), selectedEntry: checkpoint.selectedEntry }, headCommit: headSha, baseCommit: baseSha, independentReviewInput: reviewInput, reviewRepositoryPath: repositoryPath, applyEvidence, evidenceCurrent: true, evidenceReference: "pr-evidence", evidenceHeadCommit: headSha, recovery: "recover", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer, reviewRecordId: "review-1", independentReviewEvidence: evidence } }).issues[0]?.code, "derived-transition-out-of-order");
  assert.equal(checkOperationAuthorization({ authorization, runtime, config, request: { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, checkpoint: { ...checkpoint, steps: [{ id: "pr", status: "pending" }, { id: "merge-pr", status: "pending" }] }, headCommit: headSha, baseCommit: baseSha, independentReviewInput: reviewInput, reviewRepositoryPath: repositoryPath, evidenceCurrent: true, evidenceReference: "pr-evidence", evidenceHeadCommit: headSha, recovery: "recover", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer, reviewRecordId: "review-1", independentReviewEvidence: evidence } }).issues[0]?.code, "derived-checkpoint-schema-invalid");
  assert.equal(checkOperationAuthorization({ authorization, runtime, config, request: {
    profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, headCommit: headSha, baseCommit: baseSha, applyEvidence, evidenceCurrent: true, evidenceReference: "pr-evidence", evidenceHeadCommit: headSha, recovery: "re-read PR and checkpoint", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer, checkpoint, reviewRecordId: "review-1", independentReviewInput: { ...reviewInput, diff: "tampered" }, reviewRepositoryPath: repositoryPath, independentReviewEvidence: evidence
  } }).issues[0]?.code, "independent-review-diff-provenance-mismatch");
  assert.equal(checkOperationAuthorization({ authorization, runtime, config, request: { profile: "sdd-delivery", operation: "run-lifecycle-action", lifecycleAction: "merge-pr", target: "pr:42", selectedEntry: "first-change", derivedRecord: record, checkpoint: { ...derivedCheckpoint, selectedEntry: { ...derivedCheckpoint.selectedEntry, applyEvidenceRecords: [applyEvidence] } }, headCommit: headSha, baseCommit: baseSha, applyEvidence, evidenceCurrent: true, evidenceReference: "pr-evidence", evidenceHeadCommit: headSha, recovery: "recover", deliveryProfile: "production-rapid", implementerSession: "implementer", reviewer, independentReviewInput: reviewInput, reviewRepositoryPath: repositoryPath, independentReviewEvidence: evidence } }).issues[0]?.code, "independent-review-evidence-not-durable");
