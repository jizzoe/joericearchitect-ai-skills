...
test("requires matching kinds for archive and merged-branch cleanup", () => {
  ...
  assert.equal(checkOperationAuthorization({ authorization, runtime, request: { ...base, lifecycleAction: "archive-change", target: "change:first-change", derivedRecord: change, checkpoint: { selectedEntry: { name: "first-change", records: [change] }, steps: [{ id: "archive-change", status: "pending" }] }, evidenceReference: "change-evidence" } }).allowed, true);
  assert.equal(checkOperationAuthorization({ authorization, runtime, request: { ...base, lifecycleAction: "delete-merged-topic-branch", target: "branch:feature/first-change", derivedRecord: branch, checkpoint: { selectedEntry: { name: "first-change", records: [branch] }, steps: [{ id: "delete-merged-topic-branch", status: "pending" }] }, headCommit: headSha, evidenceReference: "branch-evidence", evidenceHeadCommit: headSha } }).allowed, true);
});
...
test("derived declarations require durable records for Sync as well as high-impact actions", () => {
  ...
  assert.equal(checkOperationAuthorization({ authorization: { ...authorization, targets: ["sync:first-change"] }, runtime, request: { ...request, derivedRecord: undefined, selectedEntry: undefined, checkpoint: { selectedEntry: { name: "first-change", records: [sync] }, steps: [{ id: "sync-change", status: "pending" }] } } }).issues[0]?.code, "derived-record-not-durable");
  assert.equal(checkOperationAuthorization({ authorization, runtime, request: { ...request, checkpoint: { selectedEntry: { name: "first-change", records: [sync] }, steps: [{ id: "sync-change", status: "pending" }] } } }).allowed, true);
  assert.equal(checkOperationAuthorization({ authorization, runtime, request: { ...request, derivedRecord: record, target: "pr:42", checkpoint: { selectedEntry: { name: "first-change", records: [record] }, steps: [{ id: "sync-change", status: "pending" }] } } }).issues[0]?.code, "derived-record-not-durable");
});
