...
function resourceEligible(resource, { selectedEntry, repository, deliveryEvidence }) {
  if (!exactResource(resource, { selectedEntry, repository }) ||
      resource.deliveryEvidence.reference !== deliveryEvidence.reference || resource.deliveryEvidence.deliveredHeadCommit !== deliveryEvidence.headCommit) return false;
  if (resource.kind === "worktree") return !resource.primary && !resource.locked && resource.registered === true && resource.clean === true;
  return resource.referencedElsewhere !== true && (resource.ancestryMerged === true || exactMergedPullRequest(resource.squashOrRebaseEvidence, resource.headCommit, deliveryEvidence.headCommit));
}
...
export function planWorkspaceCleanup({ selectedEntry, repository, archiveVisible, issueClosed, projectDone = true, deliveryEvidence, resources = [] } = {}) {
  ...
  const planned = resources.map((resource) => {
    if (!exactResource(resource, { selectedEntry, repository }) || resource.deliveryEvidence.reference !== deliveryEvidence.reference || resource.deliveryEvidence.deliveredHeadCommit !== deliveryEvidence.headCommit) return ineligible(resource, "cleanup-resource-record-invalid");
    ...
    if (exactMergedPullRequest(resource.squashOrRebaseEvidence, resource.headCommit, deliveryEvidence.headCommit)) return { id: resource.id, classification: "eligible", actions: [{ kind: "delete-local-branch", id: resource.id, force: true, resource, deliveryEvidence }] };
