...
export function prepareIndependentReview({ reviewer, implementerSession, reviewInput = {} }) {
  ...
  if (!nonEmpty(baseCommit) || !nonEmpty(headCommit) || !nonEmpty(diff) ||
      ...
      !Array.isArray(validationEvidence) || validationEvidence.length === 0) {
    return fail("independent-review-input-incomplete");
  }
  return { allowed: true, classification: "ready", issues: [], reviewInput: {
...
export function validateIndependentReviewEvidence({ reviewer, implementerSession, expectedBase, expectedHead, evidence }) {
  if (!reviewer?.available) return fail("independent-reviewer-unavailable");
