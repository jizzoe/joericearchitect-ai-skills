...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--auth-context-execution-context") {
      const context = argv[++index];
      if (!["host", "restricted"].includes(context)) throw new Error("auth context execution context must be host or restricted");
      args.authContextExecutionContext = context;
    }
